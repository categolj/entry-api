...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'fk_entry_categories_entry_id') THEN
        ALTER TABLE entry_categories ADD CONSTRAINT fk_entry_categories_entry_id FOREIGN KEY (entry_id) REFERENCES entry(id);
    END IF;
END$$;;
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'fk_entry_tags_entry_id') THEN
        ALTER TABLE entry_tags ADD CONSTRAINT fk_entry_tags_entry_id FOREIGN KEY (entry_id) REFERENCES entry(id);
    END IF;
END$$;;
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'fk_entry_tokens_entry_id') THEN
        ALTER TABLE entry_tokens ADD CONSTRAINT fk_entry_tokens_entry_id FOREIGN KEY (entry_id) REFERENCES entry(id);
    END IF;
END$$;;
